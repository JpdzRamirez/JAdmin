    ...
    _eventHandlers() {
        ...
        this.element.querySelectorAll('.dynamic-select-option').forEach(option => {
            option.onclick = () => {
                ...
                if(this.element.querySelector('input').id=='countryBornSelector'){
                    //si ya hay una seleccion, en el phone root
                    Livewire.dispatch('selectorCountryBornCharger', { optionSelected: option.getAttribute('data-value')});
